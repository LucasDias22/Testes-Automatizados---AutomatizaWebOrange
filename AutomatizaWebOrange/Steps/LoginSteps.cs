...
namespace AutomatizaWebOrange.Steps
{
    public class LoginSteps : Inicializacao
    {
        ...
        public static void ValidarExibicaoDaTelaPIM(string Username, string password)
        {
            Login(Username, password);
            WriteLine("Clicar no botão [PIM]");
            Driver.FindElement(DashboardPage.btPIM).Click();
            Thread.Sleep(2000);
            WriteLine("Validar se está na tela PIM");
            string pgPIMAtual = Convert.ToString(Driver.Url);
            string pgPIMEsperado = Convert.ToString("https://opensource-demo.orangehrmlive.com/web/index.php/pim/viewEmployeeList");
            Assert.AreEqual(pgPIMEsperado, pgPIMAtual, "Página carregada com sucesso!");
